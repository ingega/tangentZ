...
@print_func_text
def make_tangentz_entries(entries):
    ...
    for p in range(len(entries)):
        ...
        # strategies can be reverse or normal way
        if data.reverse:  # side change
            if side == 'BUY':
                side = "SELL"
            else:
                side = "BUY"
    ...
    while True:
        try:
            ...
            while True: # it's an error prevent
                ...
                # some hours are forbbiden, depends on BT
                ...
                if hour == data.forbidden_hour:
                    ...
                    df_in=g['df_in']
                    if len(df_in) > 0:
                        msg = f"We have {len(df_in)} tickers that reach a {data.sistema} values, those are \n {df_in['ticker']}"
                        escribirlog(msg)
                        miMail(msg)
                        make_tangentz_entries(df_in)
